        ...
        <template>
          <v-carousel v-if="items.length > 0" hide-controls class="carousel-promos">
            ...
            <v-card color="primary" flat>
              ...
              <v-card-actions>
                <v-btn flat block color="red" v-on:click="addPromo(item.id_promocion)"><v-icon dark left>favorite</v-icon>USAR PROMO</v-btn>
              </v-card-actions>
            </v-card>
          </v-carousel-item>
        </v-carousel>
...
import { getPromos, PROMO, setMiPromo } from '../core/promos.js'
...
export default {
  ...
  mounted: function () {
    ...
    getPromos(self, ['id_categoria=' + PROMO], function (result) {
      self.items = result
  ...
  methods: {
    addPromo (id) {
      ...
      setMiPromo(this, {'id_promocion': id, 'id_usuario': this.userId}, function (result) {
        if (result.id > 0) {
          self.$router.push('/promo/' + result.id)
        } else {
